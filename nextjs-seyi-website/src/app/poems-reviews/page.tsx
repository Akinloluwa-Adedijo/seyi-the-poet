...
const ListItem = ({ onItemClick, item }: ListItemProps) => {
  ...
  return (
    <ul
      ref={scope}
      key={item.id}
      className="group relative grid grid-cols-3 sm:grid-cols-4 w-full border-t-2 border-accent uppercase p-2 gap-5 cursor-pointer overflow-hidden"
      onMouseEnter={animateIn}
      onMouseLeave={animateOut}
      onClick={() => onItemClick(item)}
    >
      <li className="z-3 font-medium group-hover:text-bg transition-colors duration-300">
        {item.name} {item.type === "review" && "by " + item.artist}
      </li>
      <li className="z-3 font-medium group-hover:text-bg transition-colors duration-300">
        {item.type}
      </li>
      ...
      <li className="hidden sm:block sm:text-right read-text z-3 font-medium group-hover:text-bg transition-colors duration-300">
